  ...
  async function fetchTemplateDetails() {
    try {
      ...
      if (res.ok) {
        ...
        setError("Failed to locate template records.");
      }
    } catch (err) {
      console.error(err);
      setError("An error occurred while loading template data.");
    } finally {
      setFetching(false);
    }
  }
    ...
    <div className="flex bg-zinc-50 min-h-screen font-sans">
      ...
      <main className="flex-1 p-8 md:p-10 max-w-4xl mx-auto overflow-hidden">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-500 mt-2">Modify base blueprints for contracts, agreements, notary affidavits, or standard case drafts.</p>
        </div>
        ...
        {fetching ? (
          <div className="bg-white border border-zinc-200 p-8 rounded-2xl shadow-sm text-center py-20 flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-sm text-zinc-500 font-medium">Opening template files...</p>
          </div>
        ) : (
