...
export default function EditInvoicePage({ params }) {
  ...
  const [fetchingDirectories, setFetchingDirectories] = useState(true);
  ...
  async function fetchInvoiceDetails() {
    try {
      ...
      if (res.ok) {
        ...
        setDueDate(formatDateLocal(data.dueDate));
      ...
      console.error(err);
      setError("An error occurred while loading invoice data.");
    } finally {
    ...
    <div className="flex bg-zinc-50 min-h-screen font-sans">
      ...
      <main className="flex-1 p-8 md:p-10 max-w-4xl mx-auto overflow-hidden">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-500 mt-2">Modify advocate fees, update transaction status, or recalculate tax breakdowns.</p>
        </div>

        {/* Form Box */}
        <div className="bg-white border border-zinc-200/85 p-8 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
          {fetchingDirectories || fetchingInvoice ? (
            <div className="py-20 flex flex-col items-center justify-center gap-3">
              <div className="h-8 w-8 border-4 border-zinc-300 border-t-zinc-950 rounded-full animate-spin" />
              <p className="text-sm text-zinc-500 font-medium">Opening invoice files...</p>
            </div>
          ) : (
